...
JavaScript的数据是动态类型，运行时可以动态改变变量所标识的数据类型。然而程序中仍然有很多情况需要判断当前变量的数据类型。那么有什么有效的方法吗？
...
## toString
`toString`这儿指的是全局的函数，在浏览器中是`window.toString`，在node.js中指的是`global.toString`。对目标变量运行toString也能打印目标变量的数据类型，测试如下：
```javascript
